...
public class ContinuousAimCommand extends Command {
    ...
    public void execute() {
        ...
        Translation2d aimPoint = target.minus(velocity).minus(TurretConstants.turretPositionInRobot);
    ...
    private Angle calculateAimAngleRate(
        ...
        double t_f = Math.sqrt((X * Math.tan(theta.in(Units.Radians)) - Y) / 4.9);
        ...
        double d_t_f = (1 / (2 * Math.sqrt((X * Math.tan(theta.in(Units.Radians)) - Y) / 4.9)))
                * (((vx * Math.tan(theta.in(Units.Radians))) + (X * Math.pow(sec(theta.in(Units.Radians)), 2)) - vy)
                        / 4.9);
        ...
        return Angle.ofRelativeUnits(d_alpha_dt, Units.Radians);
    }
}
