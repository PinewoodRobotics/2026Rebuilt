import { fromQuaternionNoRoll_ZYX, VectorUtil } from "../../util/math";

export const rebuilt_welded_field = {
  1: {
    position: VectorUtil.fromArray([11.8779798, 7.4247756, 0.889]),
    rotation: fromQuaternionNoRoll_ZYX([6.123233995736766e-17, 0.0, 0.0, 1.0]),
  },
  2: {
    position: VectorUtil.fromArray([11.9154194, 4.638039999999999, 1.12395]),
    rotation: fromQuaternionNoRoll_ZYX([
      0.7071067811865476, 0.0, 0.0, 0.7071067811865476,
    ]),
  },
  3: {
    position: VectorUtil.fromArray([11.3118646, 4.3902376, 1.12395]),
    rotation: fromQuaternionNoRoll_ZYX([6.123233995736766e-17, 0.0, 0.0, 1.0]),
  },
  4: {
    position: VectorUtil.fromArray([11.3118646, 4.0346376, 1.12395]),
    rotation: fromQuaternionNoRoll_ZYX([6.123233995736766e-17, 0.0, 0.0, 1.0]),
  },
  5: {
    position: VectorUtil.fromArray([11.9154194, 3.4312351999999997, 1.12395]),
    rotation: fromQuaternionNoRoll_ZYX([
      -0.7071067811865475, -0.0, 0.0, 0.7071067811865476,
    ]),
  },
  6: {
    position: VectorUtil.fromArray([11.8779798, 0.6444996, 0.889]),
    rotation: fromQuaternionNoRoll_ZYX([6.123233995736766e-17, 0.0, 0.0, 1.0]),
  },
  7: {
    position: VectorUtil.fromArray([11.9528844, 0.6444996, 0.889]),
    rotation: fromQuaternionNoRoll_ZYX([1.0, 0.0, 0.0, 0.0]),
  },
  8: {
    position: VectorUtil.fromArray([12.2710194, 3.4312351999999997, 1.12395]),
    rotation: fromQuaternionNoRoll_ZYX([
      -0.7071067811865475, -0.0, 0.0, 0.7071067811865476,
    ]),
  },
  9: {
    position: VectorUtil.fromArray([
      12.519177399999998, 3.6790375999999996, 1.12395,
    ]),
    rotation: fromQuaternionNoRoll_ZYX([1.0, 0.0, 0.0, 0.0]),
  },
  10: {
    position: VectorUtil.fromArray([12.519177399999998, 4.0346376, 1.12395]),
    rotation: fromQuaternionNoRoll_ZYX([1.0, 0.0, 0.0, 0.0]),
  },
  11: {
    position: VectorUtil.fromArray([12.2710194, 4.638039999999999, 1.12395]),
    rotation: fromQuaternionNoRoll_ZYX([
      0.7071067811865476, 0.0, 0.0, 0.7071067811865476,
    ]),
  },
  12: {
    position: VectorUtil.fromArray([11.9528844, 7.4247756, 0.889]),
    rotation: fromQuaternionNoRoll_ZYX([1.0, 0.0, 0.0, 0.0]),
  },
  13: {
    position: VectorUtil.fromArray([16.5333172, 7.4033126, 0.55245]),
    rotation: fromQuaternionNoRoll_ZYX([6.123233995736766e-17, 0.0, 0.0, 1.0]),
  },
  14: {
    position: VectorUtil.fromArray([16.5333172, 6.9715126, 0.55245]),
    rotation: fromQuaternionNoRoll_ZYX([6.123233995736766e-17, 0.0, 0.0, 1.0]),
  },
  15: {
    position: VectorUtil.fromArray([16.5329616, 4.3235626, 0.55245]),
    rotation: fromQuaternionNoRoll_ZYX([6.123233995736766e-17, 0.0, 0.0, 1.0]),
  },
  16: {
    position: VectorUtil.fromArray([16.5329616, 3.8917626, 0.55245]),
    rotation: fromQuaternionNoRoll_ZYX([6.123233995736766e-17, 0.0, 0.0, 1.0]),
  },
  17: {
    position: VectorUtil.fromArray([4.6630844, 0.6444996, 0.889]),
    rotation: fromQuaternionNoRoll_ZYX([1.0, 0.0, 0.0, 0.0]),
  },
  18: {
    position: VectorUtil.fromArray([4.6256194, 3.4312351999999997, 1.12395]),
    rotation: fromQuaternionNoRoll_ZYX([
      -0.7071067811865475, -0.0, 0.0, 0.7071067811865476,
    ]),
  },
  19: {
    position: VectorUtil.fromArray([
      5.229174199999999, 3.6790375999999996, 1.12395,
    ]),
    rotation: fromQuaternionNoRoll_ZYX([1.0, 0.0, 0.0, 0.0]),
  },
  20: {
    position: VectorUtil.fromArray([5.229174199999999, 4.0346376, 1.12395]),
    rotation: fromQuaternionNoRoll_ZYX([1.0, 0.0, 0.0, 0.0]),
  },
  21: {
    position: VectorUtil.fromArray([4.6256194, 4.638039999999999, 1.12395]),
    rotation: fromQuaternionNoRoll_ZYX([
      0.7071067811865476, 0.0, 0.0, 0.7071067811865476,
    ]),
  },
  22: {
    position: VectorUtil.fromArray([4.6630844, 7.4247756, 0.889]),
    rotation: fromQuaternionNoRoll_ZYX([1.0, 0.0, 0.0, 0.0]),
  },
  23: {
    position: VectorUtil.fromArray([4.5881798, 7.4247756, 0.889]),
    rotation: fromQuaternionNoRoll_ZYX([6.123233995736766e-17, 0.0, 0.0, 1.0]),
  },
  24: {
    position: VectorUtil.fromArray([4.2700194, 4.638039999999999, 1.12395]),
    rotation: fromQuaternionNoRoll_ZYX([
      0.7071067811865476, 0.0, 0.0, 0.7071067811865476,
    ]),
  },
  25: {
    position: VectorUtil.fromArray([4.0218614, 4.3902376, 1.12395]),
    rotation: fromQuaternionNoRoll_ZYX([6.123233995736766e-17, 0.0, 0.0, 1.0]),
  },
  26: {
    position: VectorUtil.fromArray([4.0218614, 4.0346376, 1.12395]),
    rotation: fromQuaternionNoRoll_ZYX([6.123233995736766e-17, 0.0, 0.0, 1.0]),
  },
  27: {
    position: VectorUtil.fromArray([4.2700194, 3.4312351999999997, 1.12395]),
    rotation: fromQuaternionNoRoll_ZYX([
      -0.7071067811865475, -0.0, 0.0, 0.7071067811865476,
    ]),
  },
  28: {
    position: VectorUtil.fromArray([4.5881798, 0.6444996, 0.889]),
    rotation: fromQuaternionNoRoll_ZYX([6.123233995736766e-17, 0.0, 0.0, 1.0]),
  },
  29: {
    position: VectorUtil.fromArray([0.0077469999999999995, 0.6659626, 0.55245]),
    rotation: fromQuaternionNoRoll_ZYX([1.0, 0.0, 0.0, 0.0]),
  },
  30: {
    position: VectorUtil.fromArray([0.0077469999999999995, 1.0977626, 0.55245]),
    rotation: fromQuaternionNoRoll_ZYX([1.0, 0.0, 0.0, 0.0]),
  },
  31: {
    position: VectorUtil.fromArray([0.0080772, 3.7457125999999996, 0.55245]),
    rotation: fromQuaternionNoRoll_ZYX([1.0, 0.0, 0.0, 0.0]),
  },
  32: {
    position: VectorUtil.fromArray([0.0080772, 4.1775126, 0.55245]),
    rotation: fromQuaternionNoRoll_ZYX([1.0, 0.0, 0.0, 0.0]),
  },
};
